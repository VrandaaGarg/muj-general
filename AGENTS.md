...
2. **Admin subpage breadcrumbs + width**: Convert `ArrowLeft` back-links to full `Home > Admin > Page` breadcrumbs and widen from `max-w-3xl`/`max-w-4xl` to `max-w-7xl` with `lg:px-20` on: `/admin/users`, `/admin/history`, `/admin/departments`, `/admin/tags`, `/admin/journals`, `/admin/review/[id]`.

...
- **Admin subpage breadcrumbs + width**: 6 admin sub-routes still use `ArrowLeft` back-links instead of full `Home > Admin > Page` breadcrumbs, and narrower containers (`max-w-3xl`/`max-w-4xl`) instead of `max-w-7xl` with `lg:px-20`. Affected: `/admin/users`, `/admin/history`, `/admin/departments`, `/admin/tags`, `/admin/journals`, `/admin/review/[id]`.
